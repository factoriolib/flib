...
-- @module position
-- @alias Position
-- @usage local position = require('__flib__/position')
-- @see area
...
local string = require('__flib__/string')
local math = require('__flib__/math')

local floor, abs, atan2 = math.floor, math.abs, math.atan2
local deg, acos, max, min = math.deg, math.acos, math.max, math.min
local cos, sin, ceil, sqrt, pi, random = math.cos, math.sin, math.ceil, math.sqrt, math.pi, math.random
local round_to, round, is_number = math.round_to, math.round, math.is_number
local split = string.split
local dirs = defines.direction

local AREA_PATH = '__flib__/area'
...
-- @usage M.new({0, 0}) -- returns {x = 0, y = 0}
...
-- @treturn Concepts.Position
function M.from_string(pos_string)
  return M(load('return ' .. pos_string)())
end

--- Converts a string key position to a position.
-- @tparam string pos_string the position to convert
-- @treturn Concepts.Position
function M.from_key(pos_string)
...
-- @treturn Concepts.Position
function M.closest(pos1, positions)
  local x, y = pos1.x, pos1.y
  local closest = math.MAXINT32
  for _, pos in pairs(positions) do
    local distance = M.distance(pos1, pos)
...
-- @tparam array positions array of Concepts.Position
-- @treturn Concepts.Position
function M.farthest(pos1, positions)
  local x, y = pos1.x, pos1.y
  local closest = 0
...
function M.projection(pos1, pos2)
...
function M.reflection(pos1, pos2)
...
-- @tparam Concepts.Position pos
-- @treturn Concepts.Position
function M.floor(pos)
  return new(floor(pos.x), floor(pos.y))
end

--- Floor in place.
function M.floored(pos)
...
--- Flip the x sign
function M.flip_x(pos)
  return new(-pos.x, pos.y)
end

--- Flip the y sign
function M.flip_y(pos)
  return new(pos.x, -pos.y)
end
...
function M.lerp(pos1, pos2, alpha)
...
function M.offset_along_line(pos1, pos2, distance_from_pos2)
  distance_from_pos2 = distance_from_pos2 or 0

  local angle = M.atan2(pos1, pos2)
  local veclength = M.distance(pos1, pos2) - distance_from_pos2
...
function M.translate(pos, direction, distance)
  direction = direction or 0
  distance = distance or 1
  return M.add(pos, Direction.to_vector(direction, distance))
end
...
function M.random(pos, minimum, maximum, random_tile)
...
-- @treturn Concepts.Position a new position
function M.intersection(pos1_start, pos1_end, pos2_start, pos2_end)
...
-- @usage local chunk_x = M.chunk_position(pos).x
function M.to_chunk_position(pos)
...
-- @tparam Concepts.Position pos
-- @treturn Concepts.Position
function M.from_chunk_position(pos)
  local x, y = (floor(pos.x) * 32), (floor(pos.y) * 32)
  return new(x, y)
end
-- ))
...
local function load_area(area)
  ...
  if not Area then
    error('Area for Position not found in package.loaded. Area should be required before Position')
  end
  return Area.load(area)
end
...
function M.expand_to_area(pos, radius)
  radius = radius or 1
...
function M.to_area(pos, width, height)
  width = width or 0
...
-- @tparam Concepts.Position pos2
-- @treturn number
function M.angle(pos1, pos2)
  local dist = M.distance(pos1, pos2)
  if dist ~= 0 then
...
-- @tparam Concepts.Position pos2
-- @treturn boolean
function M.less_than(pos1, pos2)
  return M.len(pos1) < M.len(pos2)
end
...
-- @tparam Concepts.Position pos2
-- @treturn boolean
function M.less_than_eq(pos1, pos2)
  return M.len(pos1) <= M.len(pos2)
end
...
function M.distance_squared(pos1, pos2)
...
function M.direction_to(pos1, pos2)
...
-- @treturn defines.direction
function M.complex_direction_to(pos1, pos2, eight_way)
  return Orientation.to_direction(M.orientation_to(pos1, pos2), eight_way)
end

function M.orientation_to(pos1, pos2)
  return (1 - (M.atan2(pos1, pos2) / pi)) / 2
end
...
function M.increment(pos, inc_x, inc_y, increment_initial)
...
-- @table Metamethods
metatable = {
  __index = M, -- If key is not found, see if there is one availble in the Position module.
  __add = M.add, -- Adds two position together. Returns a new position.
  __sub = M.subtract, -- Subtracts one position from another. Returns a new position.
  __mul = M.multiply, -- Multiply 2 positions. Returns a new position.
  __div = M.divide, -- Divide 2 positions. Returns a new position.
  __mod = M.mod, -- Modulo of 2 positions. Returns a new position.
  __unm = M.unary, -- Unary Minus of a position. Returns a new position.
  __len = M.len, -- Length of a single position.
  __eq = M.equals, -- Are two positions at the same spot.
  __lt = M.less_than, -- Is position1 less than position2.
  __le = M.less_than_eq, -- Is position1 less than or equal to position2.
  __tostring = M.to_string, -- Returns a string representation of the position
  __call = M.new, -- copy the position.
