...
local string_split = require('__stdlib__/stdlib/utils/string').split
...
function M.from_string(area_string)
  return M(load('return ' .. area_string)())
end

--- Converts a string key area to an area.
-- @tparam string area_string the area to convert
-- @treturn Concepts.BoundingBox
function M.from_key(area_string)
  local tab = string_split(area_string, ',', false, tonumber)
...
function M.flip(area)
  local _, w, h = M.size(area)
  if w == h then
    return area -- no point flipping a square
  elseif h > w then
    local rad = h / 2 - w / 2
    return M.adjust(area, {rad, -rad})
  elseif w > h then
    local rad = w / 2 - h / 2
    return M.adjust(area, {-rad, rad})
  end
...
function M.non_zero(area, amount)
  amount = amount or 0.01
  return M.size(area) == 0 and M.expand(area, amount) or area
end
...
-- @treturn Concepts.BoundingBox
function M.to_diameter(area, diameter)
...
-- @usage local area = M.adjust({{-2, -2}, {2, 2}}, {4, -1})
...
function M.adjust(area, amount)
  local vec = Position(amount)
  area = M.new(area)

  -- shrink or expand on x vector
  if vec.x > 0 then
    area = M.expand(area, {vec.x, 0})
  elseif vec.x < 0 then
    area = M.shrink(area, {abs(vec.x), 0})
  end

  -- shrink or expand on y vector
  if vec.y > 0 then
    area = M.expand(area, {0, vec.y})
  elseif vec.y < 0 then
    area = M.shrink(area, {0, abs(vec.y)})
  end
...
function M.translate(area, direction, distance)
...
-- @tparam Concepts.BoundingBox area
-- @treturn boolean
function M.is_zero(area)
  return M.size(area) == 0
end

--- Is the area normalized.
-- @tparam Concepts.BoundingBox area
-- @treturn boolean
function M.is_normalized(area)
...
-- @tparam Concepts.BoundingBox area
-- @treturn boolean
function M.valid(area)
  return M.is_normalized(area) and M.size(area) ~= 0
end

--- Unpack an area into a tuple.
-- @tparam Concepts.Boundingbox area
-- @treturn tuple lt.x, lt.y, rb.x, rb.y
function M.unpack(area)
...
function M.unpack_positions(area)
...
function M.pack_positions(area)
...
function M.less_than(area1, area2)
  if type(area1) == 'number' then
    return area1 < M.size(area2)
  elseif type(area2) == 'number' then
    return M.size(area1) < area2
  else
    return M.size(area1) < M.size(area2)
  end
end
...
-- @local
function M.less_than_eq(area1, area2)
  if type(area1) == 'number' then
    return area1 <= M.size(area2)
  elseif type(area2) == 'number' then
    return M.size(area1) <= area2
...
function M.contains_positions(area, positions)
...
-- @treturn boolean
function M.contains_areas(area, areas)
  for _, inner in pairs(areas) do
    if not M.contains_positions(area, {M.unpack_positions(inner)}) then
      return false
    end
  end
  return true
...
-- @tparam array areas array of Concepts.BoundingBox
-- @treturn boolean
function M.collides_areas(area, areas)
  for _, inner in pairs(areas) do
    if not M.collides(area, inner) then
      return false
    end
  end
  return true
...
-- @usage
-- local area = {{0, -5}, {3, -3}}
-- for x,y in M.iterate(area) do
--   -- return x, y values
-- end
-- for position in M.iterate(area, true) do
...
function M.iterate(area, as_position, inside, step)
  step = step or 1
...
-- @usage for x, y in M.spiral_iterate({{-2, -1}, {2, 1}}) do
-- print('(' .. x .. ', ' .. y .. ')')
-- end
...
-- @treturn function an iterator
function M.spiral_iterate(area, as_position)
...
function M.positions(area, inside, step)
  local positions = {}

  for pos in M.iterate(area, true, inside, step) do
...
-- @table Metamethods
metatable = {
  __index = M, -- If key is not found see if there is one available in the Area module.
  __tostring = M.to_string, -- Will print a string representation of the area.
  ...
  __mod = __mod,
  __unm = __unm,
  __eq = M.equals, -- Is area1 the same as area2.
  __lt = M.less_than, --is the size of area1 less than number/area2.
  __le = M.less_than_eq, --is the size of area1 less than or equal to number/area2.
  __len = M.size, -- The size of the area.
  __call = M.new, -- Return a new copy.
